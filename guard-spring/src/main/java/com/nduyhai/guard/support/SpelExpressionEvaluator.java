...
/**
 * Evaluates SpEL expressions declared in Guard annotations against a
 * {@link GuardInvocationContext}.
 *
 * <p>Method parameters are exposed by name (e.g. {@code #orderId}) and the root object
 * is the bean that owns the annotated method, allowing access to {@code #root.method} etc.
 */
public final class SpelExpressionEvaluator {
    ...
    public String evaluate(String expression, GuardInvocationContext context) {
        ...
        StandardEvaluationContext evalContext = new MethodBasedEvaluationContext(
                context.getTarget(),
                context.getMethod(),
                context.getArgs(),
                nameDiscoverer);
